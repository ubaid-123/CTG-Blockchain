...
function Login(){
  var key = "3#2@3~32#3";
  const [{ userss, accounts, ownerAccount },dispatch] = useStore();
  const initialValues = {
    email: '',
    password: '',
  }
  ...
  const handleSubmit = (e) => {
    var success = false; 
    e.preventDefault()
    Object.keys(userss).forEach((key) => {
      console.log(userss[key])
      if(userss[key].email == values.email && userss[key].password == values.password){
          success = true;
          window.localStorage.setItem("success","true")
          window.localStorage.setItem("key",key)
          dispatch(initialFieldValuess((userss[key])))

          if(accounts[0] == ownerAccount){
            ...
            <div className="login-container" style={{color:'black'}}>
                <div className="login-form">
                    <div className="login-form-inner">
                        ...
                        <a className="rounded-button login-cta" 
                          onClick={handleSubmit}
                        >Sign In</a>

                        <div className="register-div">Not registered yet? 
                            <a href="/register" className="link create-account">
                                Create an account
                            </a>
                        </div>
                    </div>
                        ...
                        <div className="swiper-wrapper">
                            <div className="swiper-slide">
                                <div className="slide-content" style={{marginTop:'45%',color:'white'}}>
                                    ...
                                    <a href="/register" className="rounded-button reg-cta" style={{fontSize:"18px",fontWeight:"800"}}>Register Now</a>
                                </div>
                            </div>
                        </div>
                    </div>
